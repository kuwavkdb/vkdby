...
class UnitTest < ActiveSupport::TestCase
  ...
  test 'generating the ogp_image on first view does not bump updated_at (issue #1528)' do
    unit = Unit.create!(name: 'Touch Guard Unit', key: 'unit-ogp-touch-guard', status: :active)
    unit.update_column(:updated_at, 1.year.ago)
    original_updated_at = unit.reload.updated_at
